...
public class DefaultDeferredRenderPipeline(OpenGl openGl) : RenderPipeline(openGl)
{
    private static readonly ILogger Logger = new Logger<DefaultDeferredRenderPipeline>(LoggerFactory.Create(b => b.AddConsole()));
    ...
    public override void Render(float deltaTime, bool editor = false)
    {
        ...
        foreach (var light in Application.Scene.Lights)
        {
            ...
            if (sunLightIndex < 0 && light is { CastShadows: true, Type: LightType.Directional })
            {
                ...
                sunLight = light;
                LightSource.SunLightSource = sunLight;
            }
	...
	private unsafe void CreateShadowAtlas()
	{
	    ...
	    Gl.TexParameterI(TextureTarget.Texture2D, TextureParameterName.TextureCompareMode, (int)TextureCompareMode.CompareRefToTexture);
	    Gl.TexParameterI(TextureTarget.Texture2D, TextureParameterName.TextureCompareFunc, (int)DepthFunction.Lequal);
    ...
    public override unsafe void CreateFinalFramebuffer(Vector2D<int> viewport)
    {
        ...
        Gl.TexImage2D(TextureTarget.Texture2D, 0, InternalFormat.Rgba16f, (uint)viewport.X, (uint)viewport.Y, 0, PixelFormat.Rgba, PixelType.Float, null);
        ...
        if (Gl.CheckFramebufferStatus(FramebufferTarget.Framebuffer) != GLEnum.FramebufferComplete)
            Logger.LogWarning("Final FBO incomplete");
    ...
    {
        var geomFs = OpenGl.LoadEmbeddedResourceShaderCode("deferred_geom.frag");
        if (geomFs == null)
            throw new Exception("deferred_geom.frag not found in resources!");
        DeferredGeometryShader = new Shader(Gl, vertexWorldCode, geomFs);

        var lightVsCode = OpenGl.LoadEmbeddedResourceShaderCode("deferred_light.vert");
        var lightFsCode = OpenGl.LoadEmbeddedResourceShaderCode("deferred_light.frag");
        ...
        if (outlineFsCode == null || outlineVsCode == null)
            throw new Exception("outline.frag or outline.vert not found in resources!");
        OutlineShader = new Shader(Gl, outlineVsCode, outlineFsCode);
    }

    private void BuildShadowProgram()
    {
        var vs = OpenGl.LoadEmbeddedResourceShaderCode("shadow_depth.vert");
        var fs = OpenGl.LoadEmbeddedResourceShaderCode("shadow_depth.frag");
        if (vs == null || fs == null)
    ...
    private unsafe void CreateDeferredLightQuad()
    {
        ...
        DeferredLightVao = Gl.GenVertexArray();
        ...
        in Vector3 sunDirection)
    {
        ...
        Gl.Clear((uint)(ClearBufferMask.ColorBufferBit | ClearBufferMask.StencilBufferBit));
        ...
        if (OpenGl.PostProcessVolume.Effects.Count > 0)
        {
            Gl.Enable(EnableCap.Blend);
            Gl.BlendFunc(BlendingFactor.SrcAlpha, BlendingFactor.OneMinusSrcAlpha);
    
            foreach (var postProcessEffect in OpenGl.PostProcessVolume.Effects)
                postProcessEffect.Render(OpenGl);
        }
        ...
        Gl.ActiveTexture(TextureUnit.Texture0);
        Gl.BindTexture(TextureTarget.Texture2D, DepthTexture);
        OutlineShader.SetUniform("gDepth", 0);

        OutlineShader.SetUniform("viewportSize", new Vector2(viewport.X, viewport.Y));
        Gl.BindVertexArray(DeferredLightVao);
        Gl.DrawArrays(PrimitiveType.Triangles, 0, 3);
        Gl.BindVertexArray(0);
