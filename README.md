# FlyEngine

FlyEngine is an open-source modular game engine/framework for creating 3d games, includes netcode, powered by Silk.NET, LiteNetLib, MemoryPack, JoltPhysics and SteamAudio. FlyEngine using Unity-like GameObject system, ECS in todo list.

## TODO
- [] ECS system
- [] Finish netcode
- [] Basic editor (hierarchy, scenes, inspector)
- [] Debug gizmos
- [] Custom package system
- [] Make my first game on my game engine :tada:

## Contributing

Pull requests are welcome. For major changes, please open an issue first
to discuss what you would like to change.

## License

- [FlyEngine](https://github.com/flyen256/FlyEngine) - [MIT License](https://choosealicense.com/licenses/mit/)
- [ImGui.NET](https://github.com/ImGuiNET/ImGui.NET) - [MIT License](https://choosealicense.com/licenses/mit/)
- [Silk.NET](https://github.com/dotnet/Silk.NET) - [MIT License](https://choosealicense.com/licenses/mit/)
- [MemoryPack](https://github.com/Cysharp/MemoryPack) - [MIT License](https://choosealicense.com/licenses/mit/)
- [LiteNetLib](https://github.com/revenantx/litenetlib) - [MIT License](https://choosealicense.com/licenses/mit/)
- [JoltPhysicsSharp](https://github.com/amerkoleci/JoltPhysicsSharp) - [MIT License](https://choosealicense.com/licenses/mit/)
- [StbImageSharp](https://github.com/StbSharp/StbImageSharp) - Public Domain